...
int getPointDegree(float direction, float pointX, float pointY, float refX, float refY) {
    float targetAngle = std::atan2(pointY - refY, pointX - refX);
...
private:
    DirectionType _dir = DirectionType::kForward;
    int _nextF = 0;
    int _levelR = 0;
...
public:
    ...
    int result[3] = {0};
    // Eat
    const std::vector<Food>& foods = game.Foods();
    for (auto& food : foods) {
        float distance = sqrt(pow(head.x - food.position.x, 2) + pow(head.y - food.position.y, 2));
        if(distance > Game::kSnakeRadius * 10)
            continue;
        float angle = getPointDegree(direction, food.position.x, food.position.y, head.x, head.y);
        float score = game.FieldWidth() - distance;
    ...
    // Wall
    if(direction < 90) {
        if(game.FieldWidth() - head.x < Game::kSnakeRadius * dist_scale)
            result[0] -= (game.FieldWidth() - head.x) * wall_penalty;
        if(game.FieldHeight() - head.y < Game::kSnakeRadius * dist_scale)
            result[0] -= (game.FieldHeight() - head.y) * wall_penalty;
    } else if (direction < 180) {
        if(head.x < Game::kSnakeRadius * dist_scale)
            result[0] -= head.x * wall_penalty;
        if(game.FieldHeight() - head.y < Game::kSnakeRadius * dist_scale)
            result[0] -= (game.FieldHeight() - head.y) * wall_penalty;
    } else if (direction < 270) {
        if(head.x < Game::kSnakeRadius * dist_scale)
            result[0] -= head.x * wall_penalty;
        if(head.y < Game::kSnakeRadius * dist_scale)
            result[0] -= head.y * wall_penalty;
    } else {
        if(game.FieldWidth() - head.x < Game::kSnakeRadius * dist_scale)
            result[0] -= (game.FieldWidth() - head.x) * wall_penalty;
        if(head.y < Game::kSnakeRadius * dist_scale)
    ...
    for (auto it = game.Snakes().begin(); it != game.Snakes().end(); ++it) {
        ...
        if (anotherID == id) continue;
        for (const Position& pos : anotherSnake.Body()) {
            float distance = sqrt(pow(head.x - pos.x, 2) + pow(head.y - pos.y, 2));
            if (distance > Game::kSnakeRadius * 6) continue;
            float angle = getPointDegree(direction, pos.x, pos.y, head.x, head.y);
            float score = (game.FieldWidth() - distance);
            if(angle < 10 || angle > 350) {
                if(angle < 10)
                    result[1] -= score / 2;
